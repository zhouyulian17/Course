...
/**
 *  This class has been modified from the original StdDraw.java to create a timer 
 *  that fires action event, and to enable the detection of left click and right  
 *  click from the mouse input. Modified parts have been labeled with "//**"
 */
...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static double timerX = 0;
    private static double timerY = 0;
    private static double timerW = 0;
    private static double timerH = 0;
    
    // mouse state
    private static boolean isMousePressed = false;
    private static boolean mouseLeft = false;  //** left clicked
    private static boolean mouseRight = false; //** right clicked
...
*/
    ...
    ***************************************************************************/
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        */
    	//** detect timer event and draw the timer. 
    ...
    /** 
     *  Reset the state of timer.
     */
    public static void resetTimer(double x, double y, double w, double h) {
    	currentTime = 0;
    	timerX = x;
    	timerY = y;
    	timerW = w;
    	timerH = h;
    	timer = new Timer(1000, std);
    }
    ...
    public static void drawTimer() {
    	offscreen.setFont(font);
    	FontMetrics metrics = offscreen.getFontMetrics();
        double xs = scaleX(timerX);
        double ys = scaleY(timerY);
        double w = factorX(2*timerW);
        double h = factorY(2*timerH);
        String text;
        if (currentTime < 10) text = "00" + currentTime;
        else if (currentTime < 100) text = "0" + currentTime;
        else if (currentTime < 1000) text = "0" + currentTime;
        else text = "999";
        int ws = metrics.stringWidth(text);
        int hs = metrics.getDescent();
        setPenColor(WHITE);
        offscreen.fill(new Rectangle2D.Double(xs - w/2, ys - h/2, w, h));
        setPenColor(BLACK);
        offscreen.draw(new Rectangle2D.Double(xs - w/2, ys - h/2, w, h));
        setPenColor(RED);
        offscreen.drawString(text, (float) (xs - ws/2.0), (float) (ys + hs));
        draw();
    }
    ...
    ***************************************************************************/
    ...
    public static boolean isLeftClick() {  //** return true if left button of mouse is clicked
        synchronized (mouseLock) {
            return mouseLeft;
        }
    }
    public static boolean isRightClick() { //** return true if right button of mouse is clicked
        synchronized (mouseLock) {
            return mouseRight;
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            ...
            isMousePressed = true;
            if (SwingUtilities.isLeftMouseButton(e))       mouseLeft  = true; //** mouse left-clicked
            else if (SwingUtilities.isRightMouseButton(e)) mouseRight = true; //** mouse right-clicked
        }
    }
    ...
    public void mouseReleased(MouseEvent e) {
        synchronized (mouseLock) {
            ...
            mouseLeft  = false; //** update mouse click status
            mouseRight = false; //**
        }
    }
...
// Last updated: Fri Jul 7 11:06:52 EDT 2017.