...
class Patient(object):
    ...
    def update(self):
        ...
        self.viruses = [virus for virus in newviruses]
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for j in range(300):
            sum[j] += patient.update()
            
    avg = [float(number) / numTrials for number in sum]
    
    pylab.plot(avg)
    pylab.title("SimpleVirus simulation")
    pylab.xlabel("Time Steps")
    pylab.ylabel("Average Virus Population")
    pylab.legend("virus curve")
    pylab.show()
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in self.getViruses():
            try:
                newviruses.append(virus.reproduce(popDensity, self.drugs))
            except NoChildException:
                pass
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        for j in range(150):
            sum[j] += p.update()
            sumR[j] += p.getResistPop(['guttagonol'])
    ...
    avg = [float(number) / numTrials for number in sum]
    avgR = [float(number) / numTrials for number in sumR]
    
    pylab.figure()
    pylab.plot(avg, label = "total")
    pylab.plot(avgR, label = "resist")
